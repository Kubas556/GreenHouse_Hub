...
const dbURI = __dirname + "/greenhouse.db";
let dbNeedsSetup = false;
console.log("[database] database file location: " + dbURI);

if (!existsSync(dbURI)) {
  writeFileSync(dbURI, "");
  console.log("[database] created db file");
  dbNeedsSetup = true;
}

let db = new sqlite3.Database(dbURI, (err) => {
  if (err) {
    return console.error(err.message);
  }
  console.log("[database] Connected to the greenhouse SQlite database.");

  if (dbNeedsSetup) {
    console.log("[database] creating table");
    db.exec(
      "create table LogedIn (ID INTEGER PRIMARY KEY, email TEXT NOT NULL, password TEXT NOT NULL)",
      (error) => {
        if (error) console.log("[database] creation failed: " + error.message);
...
db.serialize(function () {
  db.get("SELECT email,password FROM LogedIn", function (err, row) {
    if (row)
      if (row.email) loginUser(row.email, row.password);
      else console.log("no loged user yet");
  });
});
...
function loginUser(email, password, webResponse) {
    ...
    auth
      ...
      .then((resp) => {
        if (resp)
          if (resp.user)
            db.run(
              "INSERT INTO LogedIn (ID,email,password) VALUES (1,$email,$password) ON CONFLICT (ID) DO UPDATE SET email=$email,password=$password;",
              {
                $email: email,
                $password: password,
              },
              function (call, err) {
                if (err) {
                  console.log("[database] inserting credentials failed");
                }
              }
            );
  ...
  if (authState === "SIGNED_IN") auth.signOut().then(login);
  else login();
}
...
function refreshOrCreateTargetTempListener(mac) {
  ...
  targetTempListeners[mac] = database.ref(
    "/users/" + lastUserUID + "/devices/" + mac + "/targetTemp"
  );
  targetTempListeners[mac].on("value", (data) => {
    //targetTemp = data.val().targetTemp;
    try {
      if (
        setTargetTempESPCommand(
          mac,
          data.val().temp,
          data.val().tempTolerantN,
          data.val().tempTolerantP
        )
      )
        console.log(
          "[server]executed setTargetTempESPCommand with temp: " +
            data.val().temp +
            " and tolerants: " +
            data.val().tempTolerantN +
            " and " +
...
function refreshOrCreateIrrigationListener(mac) {
  ...
  irrigationListeners[mac] = database.ref(
    "/users/" + lastUserUID + "/devices/" + mac + "/irrigation"
  );
  irrigationListeners[mac].on("value", (data) => {
    //targetTemp = data.val().targetTemp;
    try {
      if (
        setIrrigationESPCommand(
          mac,
          data.val().fertiliser,
          data.val().water,
          data.val().ratio,
        ...
        console.log(
          "[server]executed setIrrigationESPCommand with fertiliser: " +
            data.val().fertiliser +
            " and water: " +
            data.val().water +
            " and total: " +
...
function refreshOrCreateIrrigationSoilHumidityListener(mac) {
  ...
  irrigationSoilHumidityListeners[mac] = database.ref(
    "/users/" + lastUserUID + "/devices/" + mac + "/irrigationSoilHumidity"
  );
  irrigationSoilHumidityListeners[mac].on("value", (data) => {
    try {
      if (setIrrigationSoilHumidityESPCommand(mac, data.val()))
        console.log(
          "[server]executed setIrrigationSoilHumidityESPCommand with value: " +
            data.val()
        );
    } catch (e) {
      console.log(e);
...
auth.onAuthStateChanged((authUser) => {
  if (authUser) {
    ...
    database
      .ref("/users/" + lastUserUID + "/profile/username")
      .once("value", (data) => {
        /*server.listen(appPort,appIp, () => {
            console.log(`[server]Example app listening at http://${appIp}:${appPort}`);
         })*/

        if (!brodcastListener) brodcastListener = BrodcastListener();
...
wss.on("connection", (ws, request) => {
  if (authState !== "SIGNED_IN") return ws.terminate();
  ...
  ws.on("error", function close() {
    ...
    if (ws.length > 0) removeTargetTempListener(ws.mac);
...
function getTargetTempCommand(ws, mac) {
  try {
    database
      .ref("/users/" + lastUserUID + "/devices/" + mac + "/targetTemp")
      .once("value", (data) => {
        ws.send(
          "res|" +
            data.val().temp +
...
function getIrrigationCommand(ws, mac) {
  try {
    database
      .ref("/users/" + lastUserUID + "/devices/" + mac + "/irrigation")
      .once("value", (data) => {
        ws.send(
          "res|" +
            data.val().fertiliser +
...
function setTempCommand(mac, temp) {
  if (findIpAddress(mac).length > 0) {
    console.log(
      "[server]setTemp command executed on " +
        mac +
        " with temperature of " +
        temp
    );
    database
      .ref("/users/" + lastUserUID + "/devices/" + mac + "/temp")
      .set(temp.toString())
      .catch((error) => {
...
function setLowWaterNotify(mac, val) {
  if (findIpAddress(mac).length > 0) {
    console.log(
      "[server]set notification for low water on " + mac + " value: " + val
    );
    database
      .ref(
        "/users/" + lastUserUID + "/devices/" + mac + "/notifications/lowWater"
      )
      .set(val)
      .catch((error) => {
        console.log("[server]bad command");
...
function setSoilHumidityCommand(mac, hum) {
  if (findIpAddress(mac).length > 0) {
    console.log(
      "[server]setSoilHumidityCommand command executed on " +
        mac +
        " with humidity of " +
        hum
    );
    database
      .ref("/users/" + lastUserUID + "/devices/" + mac + "/soilHumidity")
      .set(hum.toString())
      .catch((error) => {
        console.log("[server]bad command");
      });
...
function setAirHumidityCommand(mac, hum) {
  if (findIpAddress(mac).length > 0) {
    console.log(
      "[server]setAirHumidityCommand command executed on " +
        mac +
        " with humidity of " +
        hum
    );
    database
      .ref("/users/" + lastUserUID + "/devices/" + mac + "/airHumidity")
      .set(hum.toString())
      .catch((error) => {
        console.log("[server]bad command");
      });
...
function setTargetTempESPCommand(mac, temp, toleranceN, toleranceP) {
  ...
  if (target.length > 0) {
    target[0].ws.send(
      "setTargetTemp|" + temp + "|" + toleranceN + "|" + toleranceP
    );
    return true;
  } else {
    return false;
...
function setIrrigationESPCommand(mac, fert, water, ratio, total) {
  ...
  if (target.length > 0) {
    target[0].ws.send(
      "setIrrigation|" + fert + "|" + water + "|" + ratio + "|" + total
    );
    return true;
  } else {
    return false;
...
app.get("/status", (req, resp) => {
  ...
  else resp.send("sing in please <a href='/login'>Login page</a>");
});
...
app.post("/discDev", (req, resp) => {
  let targetDevice = macToIpAddress.filter(
    (device) => device.mac === req.body.mac
  );
